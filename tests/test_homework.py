...
class TestRecord:
    ...
    def test_init(self, kwargs, msg_err):
        ...
        assert hasattr(
            result, 'amount'), msg_err(
            'add_attr', 'amount', 'Record')
        assert result.amount == kwargs['amount'], msg_err(
            'wrong_attr', 'amount', 'Record')
        assert hasattr(
            result, 'comment'), msg_err(
            'add_attr', 'comment', 'Record')
        assert result.comment == kwargs.get(
            'comment', ''), msg_err(
        ...
        if 'date' in kwargs:
            assert result.date == datetime.strptime(kwargs['date'], '%d.%m.%Y').date(), \
                msg_err('wrong_attr', 'date', 'Record',
        ...
        assert not hasattr(
            result, 'USD_RATE'), msg_err(
            'dont_create_attr', 'USD_RATE', 'Record')
        assert not hasattr(
            result, 'EURO_RATE'), msg_err(
...
class TestCalculator:
    ...
    def test_init(self, init_limit, msg_err):
        assert hasattr(
            homework, 'Calculator'), msg_err(
            'add_class', 'Calculator')
        result = homework.Calculator(init_limit)
        assert hasattr(
            result, 'limit'), msg_err(
            'add_attr', 'limit', 'Calculator')
        assert result.limit == init_limit, msg_err(
            'wrong_attr', 'limit', 'Calculator')
        assert hasattr(
            result, 'records'), msg_err(
            'add_attr', 'records', 'Calculator')
        assert result.records == [], msg_err(
            'wrong_attr', 'records', 'Calculator')

        assert not hasattr(result, 'USD_RATE'), msg_err(
            'dont_create_attr', 'USD_RATE', 'Calculator')
        assert not hasattr(result, 'EURO_RATE'), msg_err(
            'dont_create_attr', 'EURO_RATE', 'Calculator')

    def test_add_record(self, init_limit, data_records, msg_err):
        result = homework.Calculator(init_limit)
        assert hasattr(result, 'add_record'), msg_err(
            'add_method', 'add_record', 'Calculator')
        records, today, week = data_records
        for record in records:
            result.add_record(record)
        assert result.records == records, msg_err(
            'wrong_attr', 'records', 'Calculator')
    ...
    def test_get_today_stats(self, init_limit, data_records, msg_err):
        ...
        for record in records:
            result.add_record(record)
        assert hasattr(result, 'get_today_stats'), msg_err(
            'add_method', 'get_today_stats', 'Calculator')
        assert result.get_today_stats() == today, msg_err(
            'wrong_method', 'get_today_stats', 'Calculator')
    ...
    def test_get_week_stats(self, init_limit, data_records, msg_err):
        ...
        for record in records:
            result.add_record(record)
        assert hasattr(result, 'get_week_stats'), msg_err(
            'add_method', 'get_week_stats', 'Calculator')
        assert result.get_week_stats() == week, msg_err(
            'wrong_method', 'get_week_stats', 'Calculator')
...
class TestCaloriesCalculator:
    ...
    def test_init(self, init_limit, msg_err):
        ...
        assert hasattr(
            result, 'limit'), msg_err(
            'child_method', 'CaloriesCalculator', 'Calculator')
        assert result.limit == init_limit, msg_err(
            'child_method', 'CaloriesCalculator', 'Calculator')

        assert not hasattr(result, 'USD_RATE'), msg_err(
            'dont_create_attr', 'USD_RATE', 'CaloriesCalculator')
        assert not hasattr(result, 'EURO_RATE'), msg_err(
            'dont_create_attr', 'EURO_RATE', 'CaloriesCalculator')
...
class TestCashCalculator:
    ...
    def test_init(self, init_limit, msg_err):
        ...
        assert hasattr(
            result, 'limit'), msg_err(
            'child_method', 'CashCalculator', 'Calculator')
        assert result.limit == init_limit, msg_err(
            'child_method', 'CashCalculator', 'Calculator')

        assert hasattr(result, 'EURO_RATE'), msg_err(
            'add_attr', 'EURO_RATE', 'CashCalculator')
        assert isinstance(
            result.EURO_RATE, float), msg_err(
        ...
        assert hasattr(result, 'USD_RATE'), msg_err(
            'add_attr', 'USD_RATE', 'CashCalculator')
        assert isinstance(
            result.USD_RATE, float), msg_err(
            'wrong_attr', 'USD_RATE', 'CashCalculator')
